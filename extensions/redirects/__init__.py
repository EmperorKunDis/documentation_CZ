# Adaptováno z https://github.com/sphinx-contrib/redirects
    ...
    for redirects_file in redirects_dir.iterdir():
        ...
        with redirects_file.open(mode='r') as f:
            for line in f.readlines():
                # Vynechejte komentář nebo prázdné řádky.
                if not line.rstrip() or line.startswith('#'):
                    continue

                # Přidejte pravidlo pro přesměrování do řádku.
                match_result = pattern.match(line)

                # Vyloučit neplatné pravidlo.
                if not match_result:
                    logger.error(
                ...
                # Připravte zdrojové a cílové cesty.
