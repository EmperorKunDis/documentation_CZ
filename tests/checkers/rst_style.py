...
def check_heading_delimiters_characters(file, lines, options=None):
    """Zkontrolujte, zda jsou v hlavičce použity jen povolené znaky."""
...
def check_heading_delimiters_order(file, lines, options=None):
    """Zkontrolujte, zda se v pořadí správně objevují oddělovače nadpisů."""
    ...
    for lno, line in enumerate(lines):
        if HEADING_DELIMITER_RE.search(line):  # The line is a heading delimiter.
            ...
            if delimiter_char_index > last_delimiter_char_index + 1:
                # Ve znakových souborech používaných pro oddělovače nadpisů je rozdíl větší než jeden.
...
def check_max_one_main_heading(file, lines, options=None):
    """Zkontrolujte, zda je v dokumentu maximálně jedna hlavní nadpisová úroveň (h1)."""
    code = "".join(lines)
...
def check_min_one_main_heading(file, lines, options=None):
    """Zkontrolujte, zda dokument obsahuje hlavní nadpis (H1)."""
...
def check_heading_delimiters_length(file, lines, options=None):
    """Zkontrolujte, že znaky oddělující nadpis mají stejnou délku jako jejich nadpis."""
    for lno, line in enumerate(lines):
...
def check_heading_spacing(file, lines, options=None):
    """Zkontrolujte, zda nadpisy předchází a následují alespoň jedna prázdná řádka."""
    for lno, line in enumerate(lines):
        previous_line = lno > 1 and lines[lno - 1]
        if HEADING_DELIMITER_RE.search(line) and previous_line != '\n':  # Heading delimiter found.
            if MAIN_HEADING_RE.search(''.join(lines[lno:lno+3])):  # Upper delimiter of h1.
            ...
            if lno_before_heading >= 0 and lines[lno_before_heading] != '\n':
                # Hlavička nemusí být předcházena prázdnou řádkou, pokud je na první řádce souboru.
...
Poznámka: pouze příkaz make review
    """

    def is_valid_line(line_, forbidden_starting_chars_):
        """Povoleno brát dříve – manipulace s kartami a náboji"""
    ...
    def get_next_line_first_word(next_line_):
        """Vraťte první slovo následujícího řádku"""
...
def check_git_conflict_markers(file, lines, options=None):
    """Zkontrolujte, zda nejsou žádné konfliktní značky."""
    for lno, line in enumerate(lines):
